...
static void go_to_parent_dir(char *path); // 修改点：替换strip_ext函数
 ...
 *====================*/
static void format_file_size(uint32_t size, char *buf, size_t buf_len) {
    if (size < 1024) {
        lv_snprintf(buf, buf_len, "%u B", size);
    } else if (size < 1024 * 1024) {
        lv_snprintf(buf, buf_len, "%.1f KB", size / 1024.0f);
    } else {
        lv_snprintf(buf, buf_len, "%.1f MB", size / (1024.0f * 1024.0f));
    }
}

#if LV_FILE_EXPLORER_QUICK_ACCESS
void lv_file_explorer_set_quick_access_path(lv_obj_t *obj, lv_file_explorer_dir_t dir,
                                            const char *path) {
...
static void lv_file_explorer_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
    ...
    lv_obj_set_size(explorer->quick_access_area, LV_PCT(FILE_EXPLORER_QUICK_ACCESS_AREA_WIDTH),
                    LV_PCT(100));
    ...
    lv_obj_set_style_bg_color(lv_list_add_text(explorer->list_device, "DEVICE"),
    ...
    lv_obj_set_style_bg_color(lv_list_add_text(explorer->list_places, "PLACES"),
    ...
    lv_table_set_column_count(explorer->file_table, 4);
    lv_table_set_column_width(explorer->file_table, 0, LV_PCT(50));
    lv_table_set_column_width(explorer->file_table, 1, LV_PCT(20));
    lv_table_set_column_width(explorer->file_table, 2, LV_PCT(20));
    lv_table_set_column_width(explorer->file_table, 3, LV_PCT(10));
...
static void init_style(lv_obj_t *obj) {
    ...
    // 新增行高设置代码 - 设置表格行的上下内边距
    lv_obj_set_style_pad_top(explorer->file_table, 15, LV_PART_ITEMS);    // 上内边距
    lv_obj_set_style_pad_bottom(explorer->file_table, 15, LV_PART_ITEMS); // 下内边距

    // 刷新表格尺寸
    lv_obj_refresh_self_size(explorer->file_table);
}
...
static void browser_file_event_handler(lv_event_t *e) {
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if ((lv_strcmp(str_fn, "..") == 0) && (lv_strlen(explorer->current_path) > 3)) {
            go_to_parent_dir(explorer->current_path);
            lv_snprintf((char *)file_name, sizeof(file_name), "%s", explorer->current_path);
        } else {
            if (lv_strcmp(str_fn, "..") != 0) {
                lv_snprintf((char *)file_name, sizeof(file_name), "%s%s", explorer->current_path,
                            str_fn);
            }
        }
...
static void go_to_parent_dir(char *path) {
...
static void show_dir(lv_obj_t *obj, const char *path) {
    ...
    char file_info[32];
    char file_path[LV_FILE_EXPLORER_PATH_MAX_LEN];
    ...
    lv_table_set_cell_value(explorer->file_table, index, 2, "<DIR>");
    lv_table_set_cell_value(explorer->file_table, index, 3, "");
    index++;
    ...
    while (1) {
        ...
        lv_snprintf(file_path, sizeof(file_path), "%s%s", path,
                    (path[strlen(path) - 1] == '/') ? "" : "/");
        lv_strcat(file_path, fn);

        /*fn is empty, if not more files to read*/
        if (lv_strlen(fn) == 0) {
            LV_LOG_USER("Not more files to read!");
            printf("file_path: %s Not more files to read!\r\n", file_path);
            break;
        }
        ...
        lv_fs_res_t res_f;
        if (fn[0] == '/') {
            lv_strcpy(file_info, "<DIR>");
        } else if (lv_fs_open(&file, file_path, LV_FS_MODE_RD) == LV_FS_RES_OK) {
            uint32_t size = 0;
            lv_fs_seek(&file, 0, LV_FS_SEEK_END);
            lv_fs_tell(&file, &size);
            lv_fs_close(&file);

            format_file_size(size, file_info, sizeof(file_info));
        } else {
            lv_strcpy(file_info, "Unknown");
        }
        ...
        if ((is_end_with(fn, ".png") == true) || (is_end_with(fn, ".PNG") == true) ||
            ...
            lv_table_set_cell_value_fmt(explorer->file_table, index, 1, LV_SYMBOL_IMAGE "  %s", fn);
            lv_table_set_cell_value_fmt(explorer->file_table, index, 2, LV_SYMBOL_IMAGE "  %s", fn);
            lv_table_set_cell_value_fmt(explorer->file_table, index, 3, LV_SYMBOL_IMAGE "  %s", fn);
            lv_table_set_cell_value(explorer->file_table, index, 1, "1");
            lv_table_set_cell_value(explorer->file_table, index, 2, file_info);
            lv_table_set_cell_value(explorer->file_table, index, 3, file_info);
    ...
    // 确保路径以'/'结尾（除非是根目录）
    if (current_path_len > 0 && explorer->current_path[current_path_len - 1] != '/') {
    ...
    lv_label_set_text_fmt(explorer->path_label, LV_SYMBOL_EYE_OPEN " %s", explorer->current_path);
}
