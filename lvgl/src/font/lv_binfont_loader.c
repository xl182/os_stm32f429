...
int32_t load_kern(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint8_t format,
                  uint32_t start);
...
lv_font_t *lv_binfont_create(const char *path) {
    ...
    if (fs_res != LV_FS_RES_OK) {
        LV_LOG_ERROR("Failed to open font file: %s, %d\r\n", path, fs_res);
        return NULL;
    }
    ...
    if (!lvgl_load_font(&file, font)) {
        LV_LOG_ERROR("Error loading font file: %s\r\n", path);
        /*
    ...
    lv_fs_close(&file);
    custom_log("lvgl_load_font %s finished\r\n", path);
    return font;
}
...
static int read_label(lv_fs_file_t *fp, int start, const char *label) {
    ...
    if (lv_fs_read(fp, &length, 4, NULL) != LV_FS_RES_OK ||
        lv_fs_read(fp, buf, 4, NULL) != LV_FS_RES_OK || lv_memcmp(label, buf, 4) != 0) {
        LV_LOG_ERROR("Error reading '%s' label.", label);
        return -1;
    }
    buf[4] = '\0';
    custom_log("read buffer: %s, size: %d\r\n", buf, sizeof(buf));
...
static bool load_cmaps_tables(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
    ...
    if (lv_fs_read(fp, cmap_table, font_dsc->cmap_num * sizeof(cmap_table_bin_t), NULL) !=
        LV_FS_RES_OK) {
        return false;
    }
    ...
    for (unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
        ...
        case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: {
            ...
            if (cmap_table[i].format_type == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL) {
                ...
                if (lv_fs_read(fp, buf, sizeof(uint16_t) * cmap->list_length, NULL) !=
                    LV_FS_RES_OK) {
                    return false;
                }
            }
...
static int32_t load_glyph(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint32_t start,
    ...
    lv_font_fmt_txt_glyph_dsc_t *glyph_dsc = (lv_font_fmt_txt_glyph_dsc_t *)sdram_alloc(
        loca_count * sizeof(lv_font_fmt_txt_glyph_dsc_t));
    ...
    uint8_t *glyph_bmp = (uint8_t *)sdram_alloc(cur_bmp_size);
    ...
    for (unsigned int i = 1; i < loca_count; ++i) {
        ...
        if (nbits % 8 == 0) { /*Fast path*/
            ...
            glyph_bmp[cur_bmp_size + bmp_size - 1] = glyph_bmp[cur_bmp_size + bmp_size - 1]
                                                     << (nbits % 8);
        }
...
static bool lvgl_load_font(lv_fs_file_t *fp, lv_font_t *font) {
    ...
    if (header_length < 0) {
        LV_LOG_ERROR("Failed to read font header header_length\r\n");
        return false;
    } else {
        custom_log("header_length: %d\r\n", header_length);
    }
    ...
    if (cmaps_length < 0) {
        LV_LOG_ERROR("Failed to load cmaps\r\n");
        return false;
    } else {
        custom_log("cmaps length: %d\r\n", cmaps_length);
    }
    ...
    if (loca_length < 0) {
        LV_LOG_ERROR("Failed to load loca length\r\n");
        return false;
    } else {
        custom_log("loca length: %d\r\n", loca_length);
    }

    uint32_t loca_count;
    if (lv_fs_read(fp, &loca_count, sizeof(uint32_t), NULL) != LV_FS_RES_OK) {
        LV_LOG_ERROR("Failed to load loca count\r\n");
        return false;
    } else {
        custom_log("loca count: %d\r\n", loca_count);
    }
    ...
    if (font_header.index_to_loc_format == 0) {
        for (unsigned int i = 0; i < loca_count; ++i) {
            ...
            if (lv_fs_read(fp, &offset, sizeof(uint16_t), NULL) != LV_FS_RES_OK) {
                failed = true;
                LV_LOG_ERROR("Failed to load loca offset\r\n");
                break;
            }
        ...
        if (lv_fs_read(fp, glyph_offset, loca_count * sizeof(uint32_t), NULL) != LV_FS_RES_OK) {
            LV_LOG_ERROR("Failed to load loca_count * sizeof(uint32_t)\r\n");
            failed = true;
        }
    } else {
        LV_LOG_ERROR("Unknown index_to_loc_format: %d.", font_header.index_to_loc_format);
        failed = true;
    }

    if (failed) {
        LV_LOG_ERROR("Failed to load glyph_offset\r\n");
        // lv_free(glyph_offset);
        return false;
    } else {
        custom_log("success to load glyph_offset\r\n");
    }

    /*glyph*/
    uint32_t glyph_start = loca_start + loca_length;
    int32_t glyph_length =
        load_glyph(fp, font_dsc, glyph_start, glyph_offset, loca_count, &font_header);

    lv_free(glyph_offset);

    if (glyph_length < 0) {
        LV_LOG_ERROR("Failed to load glyph length\r\n");
        return false;
    } else {
        custom_log("glyph_length %d\r\n", glyph_length);
    }

    /*kerning*/
    if (font_header.tables_count < 4) {
        custom_log("no kerning\r\n");
...
int32_t load_kern(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint8_t format,
                  uint32_t start) {
    custom_log("load_kern\r\n");
    int32_t kern_length = read_label(fp, start, "kern");
    custom_log("kern length: %d\r\n", kern_length);
    if (kern_length < 0) {
        return -1;
    }
    ...
    if (lv_fs_read(fp, &kern_format_type, sizeof(uint8_t), NULL) != LV_FS_RES_OK ||
        ...
        custom_log("Failed to load kern format type\r\n");
        return -1;
    }
    custom_log("kern format type: %d\r\n", kern_format_type);
    if (0 == kern_format_type) { /*sorted pairs*/
        ...
        lv_font_fmt_txt_kern_classes_t *kern_classes =
            lv_malloc(sizeof(lv_font_fmt_txt_kern_classes_t));

        lv_memset(kern_classes, 0, sizeof(lv_font_fmt_txt_kern_classes_t));
        custom_log("sizeof(lv_font_fmt_txt_kern_classes_t) %d\r\n",
                   sizeof(lv_font_fmt_txt_kern_classes_t));
        ...
        if (lv_fs_read(fp, &kern_class_mapping_length, sizeof(uint16_t), NULL) != LV_FS_RES_OK ||
            ...
            custom_log("Failed to load kern class mapping length\r\n");
            return -1;
        }
        custom_log("kern_class_mapping_length, kern_table_rows, kern_table_cols %d %d %d\r\n",
                   kern_class_mapping_length, kern_table_rows, kern_table_cols);

        int kern_values_length = sizeof(int8_t) * kern_table_rows * kern_table_cols;
        custom_log("kern_values_length %d\r\n", kern_values_length);
        uint8_t *kern_left  = sdram_alloc(kern_class_mapping_length);
        uint8_t *kern_right = sdram_alloc(kern_class_mapping_length);
        int8_t *kern_values = sdram_alloc(kern_values_length);
        ...
        if (lv_fs_read(fp, kern_left, kern_class_mapping_length, NULL) != LV_FS_RES_OK ||
            ...
            custom_log("Failed to load kern class mapping\r\n");
            return -1;
        } else {
            custom_log("Loaded kern class mapping\r\n");
        }
    } else {
        LV_LOG_ERROR("Unknown kern_format_type: %d", kern_format_type);
        return -1;
    }
