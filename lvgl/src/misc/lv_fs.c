 ...
 *********************/
    ...
    (LV_FS_DEFAULT_DRIVE_LETTER < 'A' || 'Z' < LV_FS_DEFAULT_DRIVE_LETTER)
...
static lv_fs_res_t lv_fs_write_cached(lv_fs_file_t *file_p, const void *buf, uint32_t btw,
                                      uint32_t *bw);
...
void lv_fs_init(void) { lv_ll_init(fsdrv_ll_p, sizeof(lv_fs_drv_t *)); }

void lv_fs_deinit(void) { lv_ll_clear(fsdrv_ll_p); }
...
lv_fs_res_t lv_fs_open(lv_fs_file_t *file_p, const char *path, lv_fs_mode_t mode) {
    if (path == NULL) {
        LV_LOG_ERROR("Can't open file: path is NULL");
    ...
    if (drv == NULL) {
        LV_LOG_ERROR("Can't open file (%s): unknown driver letter, letter: %c", path, resolved_path.drive_letter);
        return LV_FS_RES_NOT_EX;
    }

    if (drv->ready_cb) {
        if (drv->ready_cb(drv) == false) {
            LV_LOG_ERROR("Can't open file (%s): driver not ready", path);
            return LV_FS_RES_HW_ERR;
        }
    }

    if (drv->open_cb == NULL) {
        LV_LOG_ERROR("Can't open file (%s): open function not exists", path);
    ...
    /* For memory-mapped files we set the file handle to our file descriptor so that we can access
     * the cache from the file operations */
...
void lv_fs_make_path_from_buffer(lv_fs_path_ex_t *path, char letter, const void *buf,
                                 uint32_t size) {
    path->path[0] = letter;
    path->path[1] = ':';
    path->path[2] = 0;
...
void lv_fs_drv_init(lv_fs_drv_t *drv) { lv_memzero(drv, sizeof(lv_fs_drv_t)); }
...
#if LV_FS_DEFAULT_DRIVE_LETTER !=                                                                  \
    '\0' /*When using default drive letter, strict format (X:) is mandatory*/
...
static lv_fs_res_t lv_fs_read_cached(lv_fs_file_t *file_p, void *buf, uint32_t btr, uint32_t *br) {
    ...
    if (start <= file_position && file_position <= end) {
        ...
        if (file_p->drv->cache_size == LV_FS_CACHE_FROM_BUFFER) {
            if (btr > buffer_remaining_length) btr = buffer_remaining_length - 1;
        }
        ...
        if (btr <= buffer_remaining_length) {
            ...
            *br = btr;
            ...
            if (btr - buffer_remaining_length > buffer_size) {
                /*If remaining data chuck is bigger than buffer size, then do not use cache, instead
                 * read it directly from FS*/
                res = file_p->drv->read_cb(file_p->drv, file_p->file_d,
                                           (char *)buf + buffer_remaining_length,
                ...
                res = file_p->drv->read_cb(file_p->drv, file_p->file_d, buffer, buffer_size,
                                           &bytes_read_to_buffer);
                file_p->cache->start = file_p->cache->end + 1;
                file_p->cache->end   = file_p->cache->start + bytes_read_to_buffer - 1;

                uint16_t data_chunk_remaining =
                    LV_MIN(btr - buffer_remaining_length, bytes_read_to_buffer);
        ...
        if (btr > buffer_size) {
            ...
            uint32_t bytes_read_to_buffer = 0;
            res = file_p->drv->read_cb(file_p->drv, file_p->file_d, (void *)buffer, buffer_size,
                                       &bytes_read_to_buffer);
...
static lv_fs_res_t lv_fs_write_cached(lv_fs_file_t *file_p, const void *buf, uint32_t btw,
    ...
    res = file_p->drv->seek_cb(file_p->drv, file_p->file_d, file_p->cache->file_position,
                               LV_FS_SEEK_SET);
    ...
    if (file_p->cache->end >= file_p->cache->start) {
        ...
        if (start_position <= file_p->cache->start && end_position >= file_p->cache->end) {
            lv_memcpy(cache_buffer, write_buffer + (file_p->cache->start - start_position),
                      file_p->cache->end + 1 - file_p->cache->start);
        } else if (start_position >= file_p->cache->start && end_position <= file_p->cache->end) {
            lv_memcpy(cache_buffer + (start_position - file_p->cache->start), write_buffer,
...
static lv_fs_res_t lv_fs_seek_cached(lv_fs_file_t *file_p, uint32_t pos, lv_fs_whence_t whence) {
    ...
    case LV_FS_SEEK_END: {
        /*Because we don't know the file size, we do a little trick: do a FS seek, then get the new
         * file position from FS*/
        res = file_p->drv->seek_cb(file_p->drv, file_p->file_d, pos, whence);
        if (res == LV_FS_RES_OK) {
...
lv_fs_res_t lv_fs_remove(const char *path) {
    if (path == NULL) {
        LV_LOG_WARN("Can't remove file: path is NULL");
        return LV_FS_RES_INV_PARAM;
    }

    resolved_path_t resolved_path = lv_fs_resolve_path(path);
    char drive_letter             = resolved_path.drive_letter;
    const char *real_path         = resolved_path.real_path;

    lv_fs_drv_t *drv = lv_fs_get_drv(drive_letter);
    if (drv == NULL) {
        LV_LOG_WARN("Can't remove file (%s): unknown driver letter", path);
        return LV_FS_RES_NOT_EX;
    }

    if (drv->ready_cb && !drv->ready_cb(drv)) {
        LV_LOG_WARN("Can't remove file (%s): driver not ready", path);
        return LV_FS_RES_HW_ERR;
    }

    if (drv->remove_cb == NULL) {
        LV_LOG_WARN("Can't remove file (%s): remove function not implemented", path);
        return LV_FS_RES_NOT_IMP;
    }
...
lv_fs_res_t lv_fs_rmdir(const char *path) {
    if (path == NULL) {
        LV_LOG_WARN("Path is NULL");
        return LV_FS_RES_INV_PARAM;
    }

    resolved_path_t resolved = lv_fs_resolve_path(path);
    char drive_letter        = resolved.drive_letter;
    const char *real_path    = resolved.real_path;

    lv_fs_drv_t *drv = lv_fs_get_drv(drive_letter);
    if (drv == NULL) {
        LV_LOG_WARN("Unknown drive letter: %c", drive_letter);
        return LV_FS_RES_NOT_EX;
    }

    if (drv->rmdir_cb == NULL) {
        LV_LOG_WARN("Driver does not support rmdir");
        return LV_FS_RES_NOT_IMP;
    }

    if (drv->ready_cb && !drv->ready_cb(drv)) {
        LV_LOG_WARN("Drive %c not ready", drive_letter);
        return LV_FS_RES_HW_ERR;
    }

    LV_PROFILER_BEGIN;
    lv_fs_res_t res = drv->rmdir_cb(drv, real_path);
    LV_PROFILER_END;
