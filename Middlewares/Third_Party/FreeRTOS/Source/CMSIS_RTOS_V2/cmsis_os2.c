...
#define SVCall_IRQ_NBR                                                                             \
    (IRQn_Type) - 5 /* SVCall_IRQ_NBR added as SV_Call handler name is not the same for CM0 and    \
                       for all other CMx */
...
#define KERNEL_VERSION                                                                             \
    (((uint32_t)tskKERNEL_VERSION_MAJOR * 10000000UL) |                                            \
     ((uint32_t)tskKERNEL_VERSION_MINOR * 10000UL) | ((uint32_t)tskKERNEL_VERSION_BUILD * 1UL))
...
static HeapRegion_t xHeapRegions[] = {{ucHeap, configTOTAL_HEAP_SIZE}, {NULL, 0}};
#else
...
/* Get OS Tick overflow status */
static uint32_t OS_Tick_GetOverflow(void) { return ((SysTick->CTRL >> 16) & 1U); }
    ...
    if (!IS_IRQ() && (func != NULL)) {
        ...
        if (attr != NULL) {
            ...
            if (attr->stack_size > 0U) {
                 ...
                 * ports.       */
                /* Stack size should be therefore 4 byte aligned in order to avoid division caused
                 * side effects */
        ...
        if (mem == 1) {
            ...
            hTask = xTaskCreateStatic((TaskFunction_t)func, name, stack, argument, prio,
                                      (StackType_t *)attr->stack_mem, (StaticTask_t *)attr->cb_mem);
#endif
        } else {
            if (mem == 0) {
#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
                if (xTaskCreate((TaskFunction_t)func, name, (uint16_t)stack, argument, prio,
                                &hTask) != pdPASS) {
                    hTask = NULL;
                }
#endif
    ...
    if (!IS_IRQ() && (max_count > 0U) && (initial_count <= max_count)) {
        ...
        if (mem != -1) {
            if (max_count == 1U) {
                ...
                if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
                    hSemaphore = xSemaphoreCreateCountingStatic(max_count, initial_count,
